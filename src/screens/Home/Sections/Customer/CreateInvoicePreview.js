      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={{ marginTop: 16 }}>
          <TouchableOpacity 
            onPress={async () => {
              try {
                const html = generateInvoiceHtml({ items, subtotal, tax, service, total, orderId });
...
// Minimal HTML generator for invoice printing/sharing
const generateInvoiceHtml = ({ items = [], subtotal = 0, tax = 0, service = 0, total = 0, orderId = '' } = {}) => {
  const rows = items.map((item, idx) => {
    ...
    return `<tr>
      ...
      <td style="padding:6px;border:1px solid #ddd;">${escapeHtml(item.name || 'Product')}</td>
      <td style="padding:6px;border:1px solid #ddd;text-align:center;">${itemQty}</td>
      <td style="padding:6px;border:1px solid #ddd;text-align:right;">${Number(itemPrice).toFixed(3)}</td>
      <td style="padding:6px;border:1px solid #ddd;text-align:right;">${Number(itemTotal).toFixed(3)}</td>
    </tr>`;
  }).join('');
  ...
  <body style="font-family: Arial, Helvetica, sans-serif; color:#111;">
    <div style="max-width:800px;margin:0 auto;padding:20px;">
      <h2 style="text-align:center;margin-bottom:4px;">Icecube Shop</h2>
      <p style="text-align:center;margin-top:0;">Invoice No: ${String(orderId || '').padStart(6,'0')}</p>
      ...
      <div style="margin-top:12px;display:flex;justify-content:space-between;">
        ...
        <div style="text-align:right;">
          <div>Subtotal: ${Number(subtotal || total).toFixed(3)} ر.ع.</div>
          <div>Tax: ${Number(tax || 0).toFixed(3)} ر.ع.</div>
          <div>Service: ${Number(service || 0).toFixed(3)} ر.ع.</div>
          <h3 style="margin-top:8px;">Grand Total: ${Number(total || subtotal).toFixed(3)} ر.ع.</h3>
        </div>
      </div>
    </div>
  </body>
